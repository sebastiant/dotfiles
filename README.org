...
Packages and services are set up both system-wide as well as via home-manager in a maybe not very clear way. The background is that I use these systems mainly via a single user.
...
- Create the first generation
  - NixOS
    #+begin_src shell
      $ nixos-rebuild switch --upgrade --flake '.#t14' --upgrade --sudo
    #+end_src
    Or if system hostname is set to nixosConfigurations.<hostname> in flake.nix outputs:
    #+begin_src shell
      $ nixos-rebuild switch --upgrade --flake --upgrade --sudo
    #+end_src
  - Linux
